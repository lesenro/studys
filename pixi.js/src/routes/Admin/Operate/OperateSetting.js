        ...
        return <div>
           {btns.map(item=>{
                ...
                if(opt.position===pos){
                    ...
                    }} key={item.id} size={opt.size} type={opt.btn_type} className={opt.className} icon={opt.icon}>{item.name}</Button>
                }else{
...
var iconsOption=iconArray.map(x=>{
    return <Option key={x} value={x}><Icon type={x} /> {x} </Option> 
});
    ...
    render() {
        ...
        return <div>
            ...
            <FormItem labelCol={{ span: 5 }}
                wrapperCol={{ span: 15 }}
                label="位置">
                <RadioGroup value={this.state.position} defaultValue={this.state.position} onChange={ev=>{
            ...
            <FormItem labelCol={{ span: 5 }}
                wrapperCol={{ span: 15 }}
                label="图标">
                <Select value={this.state.icon} defaultValue={this.state.icon} style={{ width: 200 }} onChange={ev=>{
            ...
            <FormItem labelCol={{ span: 5 }}
                wrapperCol={{ span: 15 }}
                label="按钮尺寸">
                <Select value={this.state.size} defaultValue={this.state.size} style={{ width: 200 }} onChange={ev=>{
            ...
            <FormItem labelCol={{ span: 5 }}
                wrapperCol={{ span: 15 }}
                label="类型样式">
                <Select value={this.state.btn_type} defaultValue={this.state.btn_type} style={{ width: 200 }} onChange={ev=>{
            ...
            <FormItem labelCol={{ span: 5 }}
                wrapperCol={{ span: 15 }}
                label="自定义样式">
                <Input value={this.state.className} defaultValue={this.state.className} placeholder="自定义样式名称" onChange={ev=>{
